...
int main(int argc, char **argv) {
	...
	try {
		...
		int aw = capture.get(3);
		int ah = capture.get(4);
		std::cout << "Resolution: " << aw << "x" << ah << "\n";
		if(ac::fps_force == false && input_file.size() != 0) ac::fps = capture.get(5);
		std::cout << "FPS: " << ac::fps << "\n";
		cv::Mat frame;
		capture.read(frame);
		...
		if(ac::noRecord == false) {
			writer = cv::VideoWriter(ac::fileName,-1,ac::fps, frameSize, true);
